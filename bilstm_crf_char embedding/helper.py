    ...
    with open(infile_path, "rb") as infile:
        for row in infile:
            ...
            if row_index == 1:
                ...
                emb_matrix = np.zeros((len(list(char2id.keys())), emb_dim))
                continue
            items = row.split()
            char = items[0]
...
def buildMap(train_path="train.in"):
    ...
    char2id = dict(list(zip(chars, list(range(1, len(chars) + 1)))))
    label2id = dict(list(zip(labels, list(range(1, len(labels) + 1)))))
    id2char = dict(list(zip(list(range(1, len(chars) + 1)), chars)))
    id2label = dict(list(zip(list(range(1, len(labels) + 1)), labels)))
    id2char[0] = "<PAD>"
    id2label[0] = "<PAD>"
    char2id["<PAD>"] = 0
    label2id["<PAD>"] = 0
